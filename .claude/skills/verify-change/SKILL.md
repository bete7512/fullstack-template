...
c "$B/healthz"
c "$B/readyz"
c -X POST $H -d '{"email":"live@example.com","name":"Live","password":"correct-horse-9"}' "$B/v1/users"
c -X POST $H -d '{"email":"live@example.com","name":"Live","password":"correct-horse-9"}' "$B/v1/users"
c -X POST $H -d '{"bogus":1}' "$B/v1/users"
c "$B/v1/users/not-a-number"                       # 400: ids are int64
c "$B/v1/users/999999999"                          # 404
c "$B/v1/users?limit=2"                            # default order id asc
c "$B/v1/users?sort_by=email&sort_dir=desc"          # 200
c "$B/v1/users?sort_by=password&sort_dir=sideways"   # 422 fields sort_by, sort_dir (enums not checked at bind)
...
- Take real paths/bodies from `services/api/openapi/paths/*.yaml` (prefix in `services/api/openapi/openapi.yaml`);
  the curls above are examples. Auth-protected routes need a token or expect `401`.
